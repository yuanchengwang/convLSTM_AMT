...
def pianoroll_process(raw_matrix,dual_train = False):
    ...
    if dual_train:
        ...
        pianoroll_onset[onset[0],(onset[1]+length):(onset[1]+length+4)] = 1
        pianoroll = torch.cat((pianoroll_frame,pianoroll_onset),dim = 0)
        print pianoroll.shape
    return pianoroll

def pianoroll_to_note(metrics_note,metrics_onset,min_midi_pitch, frame_length_seconds, min_duration_ms):
#	Reference:ONSETS AND FRAMES: DUAL-OBJECTIVE PIANO TRANSCRIPTION
#    frame_length_seconds = 1.0/frames_per_second
#    frame_length_seconds = frames_per_second
    pitch_start_step = {}
    sequence_interval = []
    sequence_pitch = []
    ...
    # Ensure that any frame with an onset prediction is considered active.
        metrics_note = np.logical_or(metrics_note, metrics_onset)
...
#        End an active pitch.
        ...
        if (end_time - start_time) >= min_duration_ms:
            sequence_interval.append([start_time/1000,end_time/1000])    # s is default not ms in mir_eval
            sequence_pitch.append(pitch + min_midi_pitch)        
        del pitch_start_step[pitch]

    for i, frame in enumerate(metrics_note):
        for pitch, active in enumerate(frame):
            if active:
                process_active_pitch(pitch, i)
            elif pitch in pitch_start_step:
                end_pitch(pitch, i)      
    return np.asarray(sequence_interval),np.asarray(sequence_pitch)
...
def note_measure(interval_target, pitch_target, interval_output, pitch_output,with_offset = False):
    if with_offset:
        precision,recall,F1,overlap = transcription.precision_recall_f1_overlap(interval_target, pitch_target, interval_output, pitch_output)
    else:
        precision,recall,F1,overlap = transcription.precision_recall_f1_overlap(interval_target, pitch_target, interval_output, pitch_output,offset_ratio = None)
...
def counter(data,n_note,Type): # calculte the number of note&frame per pitch
	...
	if Type == 'frame':
		note_list = np.sum(data,0)
	else:
		note_list = np.zeros((n_note,1)) 
		for note in data:
