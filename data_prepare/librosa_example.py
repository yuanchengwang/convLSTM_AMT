...
def cqt_module(y,sr=sr,bins_per_octave=bins_per_octave,n_octave=n_octave,range=RangeMIDInotes,plot=False):
	CQT_spectrum=lb.cqt(y,sr=sr,bins_per_octave=bins_per_octave,n_bins=n_octave*bins_per_octave,fmin = lb.midi_to_hz(range[0])) #real=False is no need for 0.5 and the version because there is the abs value caculation
